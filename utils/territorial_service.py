...
class TerritorialService:
    ...
    def _generate_diagnostic_recommendations(self, df: pd.DataFrame) -> str:
        ...
        recommendations = [f"📊 Analyse régionale - Région {df['code_region'].iloc[0]} :"]
    ...
    def _generate_comparison_recommendations(self, df: pd.DataFrame, region_ref: str, region_comp: str) -> str:
        ...
        recommendations = [f"🔄 Analyse comparative : Région {region_ref} vs Région {region_comp}"]
        
        # Statistiques générales
        ref_data = df[df['type_region'] == 'RÉGION_RÉFÉRENCE']
        if not ref_data.empty:
            recommendations.append(f"\nNombre de départements :")
            recommendations.append(f"- Région {region_ref}: {ref_data['nb_departements'].iloc[0]}")
            recommendations.append(f"- Région {region_comp}: {df[df['type_region'] == 'RÉGION_COMPARÉE']['nb_departements'].iloc[0]}")
