...
class PredictiveVisualization:
    ...
    def create_projection_curve(self, df: pd.DataFrame) -> Optional[go.Figure]:
        ...
        try:
            required_columns = ['type_crime', 'annee', 'projection', 'lower_bound', 
            ...
            for crime in df['type_crime'].unique():
                ...
                fig.add_trace(go.Scatter(
                    x=projected['annee'].tolist() + projected['annee'].tolist()[::-1],
                    y=projected['upper_bound'].tolist() + projected['lower_bound'].tolist()[::-1],
                    fill='toself',
            ...
            logger.error(f"Erreur lors de la création de la courbe de projection: {str(e)}")
            return None
    ...
    def create_prediction_heatmap(self, df: pd.DataFrame) -> Optional[go.Figure]:
        ...
        try:
            ...
            df_pivot = df.pivot(index='type_crime', columns='annee', values='projection')
            ...
            for year in df_pivot.columns[df_pivot.columns > last_historical]:
                variations[year] = ((df_pivot[year] - reference_values) / reference_values * 100)
    ...
    def create_risk_variations(self, df: pd.DataFrame) -> Optional[go.Figure]:
        ...
        try:
            ...
            logger.error(f"Erreur lors de la création du graphique des variations: {str(e)}")
            return None
    ...
    def create_crime_correlations(self, df: pd.DataFrame) -> Optional[go.Figure]:
        ...
        try:
            ...
            crimes_uniques = sorted(list(set(df['type_crime'].unique()) | set(df['type_crime_2'].unique())))
            ...
            logger.error(f"Erreur lors de la création de la matrice de corrélation: {str(e)}")
            return None